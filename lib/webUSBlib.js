...
export const GetDC = async ({ addNotif }) => {
  try {
    ...
    if (devices[0] && devices[0] !== undefined) {
      addNotif("Info", "Permission found");
    }
    return devices[0];
  } catch (Error) {
    addNotif("Error", "Accessing linked device failed");
  }
};
...
export const Connect = async ({ device, addNotif }) => {
  try {
    ...
    addNotif("Error", "Establishing connection failed");
    console.log(device);
    if (device) {
  ...
  const result = await device.controlTransferIn(
    {
      requestType: "vendor",
      recipient: "interface",
      request: FWREAD, // vendor-specific request:
      value: 0x00,
